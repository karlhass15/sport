...
class Data extends AbstractHelper
{
    ...
    {
        $subProduct = $this->subProductFactory->getCollection()->addFieldToFilter('product_id', ['eq' => $productId]);
    ...
    public function getSubCustomerAddress($addressId, $type)
    {
        ...
        $address = $this->address->load($addressId);
    ...
    public function getProductCustomPrice($newQty, $productId, $downloadableLinks)
    {
        ...
        if (!empty($downloadableLinks)) {
            ...
            foreach ($links as $link) {
                //$config[$link->getId()] = $link->getPrice();
        ...
        if ($baseprice == 0.0000 || $baseprice <= $discount) {
            return $baseprice;
        } else {
            if ($discount != 0.0000) {
