...
class Innapr extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $this->helper->getResourceConnection()->getConnection()->update($con->getTableName('itoris_productqa_questions'), ['inappr' => 0], ['id=?' => (int)$this->getRequest()->getParam('id')]);
            $output['success']= __('Question has been updated');
            return $this->getResponse()->setBody(\Zend_Json::encode($output));
        }catch(\Exception $e){
            $output['error']=__('Question has not been updated');
           return $this->getResponse()->setBody(\Zend_Json::encode($output));
        }
