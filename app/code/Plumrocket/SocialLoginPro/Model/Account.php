...
class Account extends \Magento\Framework\Model\AbstractModel
{
    ...
    protected function _getRandomEmail()
    {
        ...
        $address = \Plumrocket\SocialLoginPro\Helper\Data::FAKE_EMAIL_PREFIX . $this->random->getRandomString($len, $chars) .'@'. $domain;
        return $address;
    }
            ...
            if ($file = $this->_loadFile($fileUrl)) {
                if (file_put_contents($tmpPath, $file) > 0) {
                    ...
                    /*
                     ...
                     And copy file
                     */
                    $this->ioFile->mkdir($this->_photoDir . DIRECTORY_SEPARATOR . $this->_type);
                    $this->ioFile->cp(
    ...
    {
        $pathToLog = $this->dir->getPath($this->dir::LOG);
        if (!is_dir($pathToLog)) {
    ...
    protected function _getSocialModel()
    {
        ...
        $socialModel = $this->objectManager->get('Plumrocket\SocialLoginPro\Model\\' . ucfirst($this->getType()));
    ...
    public function getDebugErrors()
    {
        ...
        if (is_array($this->_callInfo)) {
            if (isset($this->_callInfo['error'])) {
                if (isset($this->_callInfo['error']['message'])) {
                    $error .= $this->_callInfo['error']['message'];
                } else {
                    $error .= $this->_callInfo['error'];
                }
            } else {
                if (isset($this->_callInfo['errors'])) {
                    if (isset($this->_callInfo['errors'][0])) {
                        $error .= $this->_callInfo['errors'][0]['message'];
                    } else {
                        $error .= $this->_callInfo['errors'];
                    }
                } else {
                    $error .= $this->protectSecretData(print_r($this->_callInfo, true));
                }
            }
        } elseif (is_object($this->_callInfo)) {
            $error .= $this->_callInfo->errors;
        } else {
            if (is_string($this->_callInfo)) {
                $error .= $this->protectSecretData($this->_callInfo);
            }
        }
        return $error;
    }
     ...
     * @return string
     */
    protected function protectSecretData($data)
    {
    ...
    public function beforeSave()
    {
        ...
        if (is_array($additional)) {
            $this->setAdditional(json_encode($additional));
        }
