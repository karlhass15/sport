...
class AddSubscriptionDetails implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if (empty($orderIds) || !is_array($orderIds)) {
            ...
            $orderData = $this->order->load($orderIds[0]);
            ...
            foreach ($orderItems as $key => $item) {
                if ($item->getIsSubscribed() == 1) {
                    ...
                    $productAddtionalOptions[$key] = $item->getProductOptions();
                    $getLabel = $productAddtionalOptions[$key]['additional_options'][2]['label'];
                    if ($getLabel == 'How Many') {
                        $howManyValue = $productAddtionalOptions[$key]['additional_options'][2]['value'];
